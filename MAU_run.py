    ...
    for epoch in range(0, args.max_epoches):
        ...
        for ims in train_input_handle:
            if itr > args.max_iterations:
                break
            # myimgs = ims[0].numpy()
            # img = np.ones((myimgs.shape[2],
            #                    myimgs.shape[0] * myimgs.shape[2],
            #                    args.img_channel))
            # res_height = myimgs.shape[2]
            # res_width = myimgs.shape[3]
            # for i in range(myimgs.shape[0]):
            #     img[:res_height,i * res_width:(i + 1) * res_width,:] = myimgs[i].reshape((64,64,1))
            #
            # cv2.imwrite("results/mau/myimgs.png", (img * 255).astype(np.uint8))
