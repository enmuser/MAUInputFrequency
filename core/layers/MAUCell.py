...
class MAUCell(nn.Module):
    ...
    def forward(self, T_t, S_t, t_att, s_att, st_att):
        ...
        st_g, st_t, st_s = torch.split(ST_concat, self.num_hidden, dim=1)
        # T_gate 为 U_t_1 第一分组
        T_gate = torch.sigmoid(t_g)
        # S_gate 为 U_s_1 第一分组
        S_gate = torch.sigmoid(s_g)

        ST_gate = torch.sigmoid(st_g)
        ...
        ST_new = ST_gate * st_s + (1 - S_gate) * t_s
        if self.cell_mode == 'residual':
            S_new = S_new + S_t
        return T_new, S_new, ST_new
