...
class MAUCell(nn.Module):
    ...
    def forward(self, T_t, T_T_t, S_t, t_att,t_t_att, s_att):
        # T_t => T(k,t-1) 当前时间特征
        # S_t => S(k-1,t) 当前空间特征
        # t_att => T(k,t-tau:t-1)
        ...
        t_t_next = self.conv_t_next(T_T_t)
        # 计算注意分数权重
        weights_list = []
        ...
        T_T_gate = torch.sigmoid(T_T_fusion)
        ...
        S_new = S_new * (1-T_T_gate) + T_T_gate * T_T_fusion
        # 如果是残差网络 将 S_t 加到 S_new 上
        if self.cell_mode == 'residual':
            S_new = S_new + S_t
        return T_new, S_new, T_T_fusion
